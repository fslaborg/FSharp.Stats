...
module NelderMead =
    ...
    let createSimplex (nmc : NmConfig) (x : Vector<float>) =    
          let n = x.Length
          let simplex =
            Array.init (n+1) 
                (fun _ -> Array.copy x )      
    ...
    let rankVertices (simplexes : Vector<float>[]) (fn : Vector<float> -> float) = 
    ...
    let update (nmc : NmConfig) (stopCounter:OptimizationStop.StopCounter) (fn : Vector<float> -> float) (nmInterResult : NmInterationResult) =   
    ...
    let initInteration (maxIteration:int) (initialsimplex :Vector<float>[]) (fn : Vector<float> -> float) = 
    ...
    let minimizeWithStopCriteria (nmc : NmConfig) (x :Vector<float>) (fn : Vector<float> -> float) (stopCriteria:OptimizationStop.StopCriteria) = 
    ...
    ///<summary> Minimize the given cost function and stop criteria </summary>
    let minimize (nmc : NmConfig) (x :Vector<float>) (fn : Vector<float> -> float) =
