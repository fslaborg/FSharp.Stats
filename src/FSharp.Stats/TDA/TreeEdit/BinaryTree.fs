...
let rec appendNodeString n s tId =
    ...
    | BinaryNode(l,id,c1,c2) ->
        ...
        let s3 =
            match c1 with
            | Empty -> ""
            | BinaryNode(_,cid,_,_) -> "    t" + tId.ToString() + "n" + id.ToString() + "->t" + tId.ToString() + "n" + cid.ToString() + ";\n"
        let s4 =
            match c2 with
            | Empty -> ""
            | BinaryNode(_,cid,_,_) -> "    t" + tId.ToString() + "n" + id.ToString() + "->t" + tId.ToString() + "n" + cid.ToString() + ";\n" 
...
let rec appendNodeMatchStrings (ml:List<int*int>) s tId1 tId2 =
    match ml with
    | [] -> s
    | head::tail -> "  t" + tId1.ToString() + "n" + (fst head).ToString() + "->" + "t" + tId2.ToString() + "n" + (snd head).ToString() + "[dir=none,ltail=cluster1,lhead=cluster2,constraint=false,color=blue,penwidth=2.0];\n" + (appendNodeMatchStrings tail s tId1 tId2)
