...
//        let (a,b) = (p.[0], p.[1])
        ...
        module Linear =
            ...
            module RTO =
                ...
                let coefficientOfVector (x: Vector<float>) (y: Vector<float>) =
                    ...
                    let numerator =
                        Seq.zip x y |> Seq.sumBy (fun (x, y) -> x * y)
                ...
                let fitFunc (coef: float) = fun x -> coef * x


                /// Fit to x
                /// coefficient is beta only
                let fit (coef: float) (x: float) = coef * x
            ...
            module Univariable =
                ...
                let coefficient (xData: Vector<float>) (yData: Vector<float>) =
                    ...
                    let X =
                        Matrix.init N 2 (fun m x -> if x = 0 then 1. else xData.[m])
                ...
                let coefficientConstrained (xData: Vector<float>) (yData: Vector<float>) ((xC, yC): float * float) =
                    ...
                    let slope =
                        RTO.coefficientOfVector xTransformed yTransformed
                ...
                let cooksDistance (xData: Vector<float>) (yData: Vector<float>) =
                    ...
                    let squaredDeviations =
                        Vector.map2 (fun y yPr -> (y - yPr) ** 2.) yPred yData

                    let MSE =
                        squaredDeviations
                        |> Vector.sum
                        |> fun sumOfSquares -> sumOfSquares / (float xData.Length)
            ...
            module Multivariable =
                ...
                let coefficients (xData: Matrix<float>) (yData: Vector<float>) =
                    ...
                    let X =
                        Matrix.init m (n + 1) (fun m n -> if n = 0 then 1. else xData.[m, n - 1])

                    Algebra.LinearAlgebra.LeastSquares X yData

                /// Fit to x
                let fit (coef: Vector<float>) (x: Vector<float>) =
                    let tmp: Vector<float> =
                        Vector.init (x.Length + 1) (fun i -> if i = 0 then 1. else x.[i - 1])
                ...
                let coefficientsCholesky (xData: Matrix<float>) (yData: Vector<float>) =
                    ...
                    let X =
                        Matrix.init (xData.NumRows) (xData.NumCols + 1) (fun i j ->
                            if j = 0 then 1.0 else xData.[i, j - 1])
        ...
        module Polynomial =
            ...
            let coefficientsWithWeighting
                ...
                if xData.Length <> yData.Length
                   || xData.Length <> weighting.Length then
            ...
            let cooksDistance order (xData: Vector<float>) (yData: Vector<float>) =
                ...
                let coeffs =
                    Algebra.LinearAlgebra.LeastSquares A yData
    ...
    module RobustRegression =
        ...
        module Linear =
            ...
            let theilSenEstimator (xData: Vector<float>) (yData: Vector<float>) =
                ...
                let indicesOfUniqueOccurences =
                    let rec loop acc i =
                        if i < xLength then
                            let tmp = xData.[i]
                            let occurences = xData |> Seq.filter (fun xT -> tmp = xT)

                            if Seq.length occurences > 1 then
                                loop acc (i + 1)
                            else
                                loop (i :: acc) (i + 1)
                        else
            ...
            let fit =
                OrdinaryLeastSquares.Linear.Univariable.fit
