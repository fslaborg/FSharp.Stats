...
let cubicInterpolationTests =
    ...
    let t2 = vector [1.0; 2.0; 3.0; 4.0]
        ...
        testCase "Natural Cubic Spline" <| fun () -> 
            ...
            let fitOutPut = tt |> Vector.map(fun x ->  CubicSpline.Simple.fit coefficientsSpline t x)
            ...
            Expect.floatClose Accuracy.low expectedValues fitOutPut "Fitted Values and Expected Output should be equal (double precision)"
        ...
        testCase "Quadratic Cubic Spline" <| fun () ->            
            ...
            let fittingFunc x = 
                CubicSpline.Simple.fit ceffs t  x           
        ...
        testCase "Akima Interpolation" <| fun () ->            
            ...
            let fittingFuncAkima x = 
                CubicSpline.Differentiable.interpolateAtX coefsAkima  x          
            Expect.floatClose Accuracy.low fittingFuncAkima 0.5 1.375 "Fitted Value should be equal (double precision)"
            Expect.floatClose Accuracy.low fittingFunc 1.0 2.0 "Fitted Value should be equal (double precision)"
            Expect.floatClose Accuracy.low fittingFunc 1.5 1.5 "Fitted Value  should be equal (double precision)" 
            Expect.floatClose Accuracy.low fittingFunc 2.5 1.953125 "Fitted Value  should be equal (double precision)"  
            Expect.floatClose Accuracy.low fittingFunc 3.5 2.484375 "Fitted Value  should be equal (double precision)" 
            Expect.floatClose Accuracy.low fittingFunc 4.5 4.136363 "Fitted Value  should be equal (double precision)" 
