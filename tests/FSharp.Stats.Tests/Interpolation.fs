    ...
    testList "Interpolation.CubicSpline" [
        testCase "Natural Cubic Spline" <| fun () -> 
            ...
            let coefficientsSpline = 
                CubicSpline.fit CubicSpline.Natural t y            
            let fitOutPut = tt |> Vector.map (CubicSpline.predict coefficientsSpline t)
        ...
        testCase "Quadratic Cubic Spline" <| fun () ->            
            let coefficientsQuadraticSpline = 
                CubicSpline.fit CubicSpline.Quadratic t2 u         
            let fittingFunc x = 
                CubicSpline.predictWithinRange coefficientsQuadraticSpline t2 x           
        ...
        testCase "Parabolic Cubic Interpolation" <| fun () ->       
            ...
            let coeffParabolic = CubicSpline.fit CubicSpline.Parabolic seriesx seriesy 
            let fittingFuncParabolic x = 
                CubicSpline.predict coeffParabolic seriesx  x     

            let genrateX = vector [20.0..25.0]
            let interpParabolic = genrateX |> Vector.map fittingFuncParabolic
            let parabolicSndDeriv x = CubicSpline.getSecondDerivative coeffParabolic seriesx x 
...
let akimaInterpolationTests =
    ...
    testList "Interpolation.CubicSpline" [
        ...
        testCase "Akima Interpolation" <| fun () ->            
            let splineCoefsAkima = Akima.fit time values 
...
let polynomialInterpolationTests =
    testList "Interpolation.Polynomial" [
        ...
        testCase "Polynomial Interpolation" <| fun() -> 
            ...
            let coeffs = Polynomial.coefficients datax datay
            let expectedCoeffs = [18489.1150794045; -249.9950165; 1.2620688143; -0.0028205075; 0.0000023552]
            let polyInterpFit x = Polynomial.fit coeffs x 
            ...
            TestExtensions.sequenceEqual(Accuracy.high) coeffs expectedCoeffs "Coefficients should be equal (double precision)"
        ]
