...
let outlierTests =
    ...
    let compareLists (a:List<float>) (b:List<float>) (str:string) =
        List.iter2 (fun a b -> Expect.floatClose Accuracy.high a b str) a b
    ...
    testList "OutlierTests"[
        testList "Z-Score"[
            ...
            testCase "Z-Scores of a population" <| fun()->
                ...
                compareLists (zScoresOfPopulation ls) zLs "Z-Score of a population was calculated incorrectly"
            ...
            testCase "Z-Scores of a sample" <| fun()->
                ...
                compareLists (zScoresOfSample ls) zLsSample "Z-Score of a sample was calculated incorrectly"
        ...
        testList "Mahalanobi's Distance"[
            testCase "Mahalanobi's Distance for an observation in a matrix"<| fun() ->
                ...
                Expect.floatClose Accuracy.high (mahalanobisDistance obs dataRow Matrix.Sample Matrix.RowWise ) 1.843936618 "Mahalanobi's Distance for an observation(Sample, RowWise) calculated incorrectly"
                Expect.floatClose Accuracy.high (mahalanobisDistance obs dataColumn Matrix.Sample Matrix.ColWise ) 1.843936618 "Mahalanobi's Distance for an observation calculated(Sample, ColWise) incorrectly"
                Expect.floatClose Accuracy.high (mahalanobisDistance obs dataRow Matrix.Population Matrix.RowWise ) 1.943679857 "Mahalanobi's Distance for an observation calculated(Population, RowWise) incorrectly"
                Expect.floatClose Accuracy.high (mahalanobisDistance obs dataColumn Matrix.Population Matrix.ColWise ) 1.943679857 "Mahalanobi's Distance for an observation calculated(Population, ColWise) incorrectly"
            ...
            testCase "Mahalanobi's Distance for every observation in a matrix"<| fun() ->
                ...
                compareLists (mahalanobisDistanceList dataRow Matrix.Sample Matrix.RowWise) mahalDistancesSample  "Mahalanobi's Distance for every observation in a matrix(Sample, RowWise) was calculated incorrectly"
                compareLists (mahalanobisDistanceList dataRow Matrix.Population Matrix.RowWise) mahalDistancesPopulation  "Mahalanobi's Distance for every observation in a matrix(Population, RowWise) was calculated incorrectly"
                compareLists (mahalanobisDistanceList dataColumn Matrix.Sample Matrix.ColWise) mahalDistancesSample "Mahalanobi's Distance for every observation in a matrix(Sample, ColWise) was calculated incorrectly"
                compareLists (mahalanobisDistanceList dataColumn Matrix.Population Matrix.ColWise) mahalDistancesPopulation  "Mahalanobi's Distance for every observation in a matrix(Population, ColWise) was calculated incorrectly"
