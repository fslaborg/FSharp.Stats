...
//create samples
let rnd = System.Random()
let norm = FSharp.Stats.Distributions.ContinuousDistribution.normal 0. 1.

///Example 1: Random samples from a standard normal distribution
let normalDistA = Array.init 200 (fun _ -> norm.Sample())
let normalDistB = Array.init 200 (fun _ -> norm.Sample())

///Example 2: Random samples from values between 0 and 1
let evenRandomA = Array.init 200 (fun _ -> rnd.NextDouble())
let evenRandomB = Array.init 200 (fun _ -> rnd.NextDouble())
...
let plotFrom2Populations sampleA sampleB =
    ...
    let qqChart =
        Chart.Point (FSharp.Stats.Signal.QQPlot.fromTwoSamples sampleA sampleB )
...
You also can plot the quantiles from a sample versus a normal distribution. Your data is z standardized prior to quantile determination

*)


//plots QQ plot from a sample population against a standard normal distribution
let plotFromOneSampleGauss sampleA =

    let qqData = FSharp.Stats.Signal.QQPlot.fromSampleToGauss sampleA
